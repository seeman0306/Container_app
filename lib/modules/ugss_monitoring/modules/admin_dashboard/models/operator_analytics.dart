...
class InspectorTaskMatrix {
  final int totalInspectors;
  final List<InspectorTaskStatus> tasks;

  InspectorTaskMatrix({required this.totalInspectors, required this.tasks});

  factory InspectorTaskMatrix.fromJson(Map<String, dynamic> json) {
    return InspectorTaskMatrix(
      totalInspectors: json['total_inspectors'] ?? 0,
      tasks: (json['tasks'] as List?)
              ?.map((e) => InspectorTaskStatus.fromJson(e))
              .toList() ??
          [],
    );
  }
...
class InspectorTaskStatus {
  ...
  factory InspectorTaskStatus.fromJson(Map<String, dynamic> json) {
    Map<String, String> safeCastMap(dynamic mapObj) {
      if (mapObj == null || mapObj is! Map) return {};
      return mapObj.map<String, String>((key, value) => MapEntry(key.toString(), value.toString()));
    }

    return InspectorTaskStatus(
