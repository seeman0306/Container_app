...
class AdminService {
  ...
  Future<InspectorTaskMatrix> getInspectorTaskMatrix({String date = ''}) async {
    final token = await TokenStorage.getToken();
    final response = await http.get(
      Uri.parse('${ApiConstants.baseUrl}/api/admin/inspector-matrix?date=$date'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
      },
    ...
    if (response.statusCode == 200) {
      return InspectorTaskMatrix.fromJson(jsonDecode(response.body));
    } else {
      throw Exception('Failed to load inspector matrix');
    }
  }
